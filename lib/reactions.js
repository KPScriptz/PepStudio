...
const LEXICON = [
  // whisper.cpp emits non-speech events as parenthetical/bracketed annotations —
  // "(laughing)", "(screaming)", "[laughter]", "(applause)", "(cheering)", "(gasps)".
  // These are the STRONGEST explicit reaction signal and were previously unscored
  // (found by running real footage: karaoke clip transcribed "(laughing)"×2 + "(screaming)"
  // yet scored 0). Match them directly.
  { re: /[([](?:laugh\w*|scream\w*|giggl\w*|chuckl\w*|cheer\w*|applause|clap\w*|gasp\w*|shout\w*|yell\w*|whoop\w*)[)\]]/gi, w: 2.6, tag: 'laugh' },
