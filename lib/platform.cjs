...
function toolDirs(appBinDir, env = process.env, platform = process.platform) {
  ...
  if (isWin(platform)) {
    ...
    if (home) {
      ...
      dirs.push(path.join(home, 'AppData', 'Local', 'Microsoft', 'WinGet', 'Links'));
    }
...
function revealCmd(filePath, platform = process.platform) {
  ...
  return { cmd: 'xdg-open', args: [path.dirname(filePath)], ignoreExit: false };
}
...
function binCandidates(base, env = process.env, platform = process.platform) {
  ...
  if (isWin(platform)) {
    if (home) {
      out.push(path.join(home, 'scoop', 'shims', name));
      out.push(path.join(home, 'AppData', 'Local', 'Microsoft', 'WinGet', 'Links', name));
    }
    if (env.ProgramData) out.push(path.join(env.ProgramData, 'chocolatey', 'bin', name));
    if (env.ProgramFiles) out.push(path.join(env.ProgramFiles, base, 'bin', name));
  } else {
...
function whisperModelDirs(env = process.env, platform = process.platform) {
  ...
  if (isWin(platform)) {
    if (env.LOCALAPPDATA) out.push(path.join(env.LOCALAPPDATA, 'PepStudio', 'models'));
    if (home) out.push(path.join(home, 'whisper.cpp', 'models'));
  } else {
    if (home) out.push(path.join(home, 'whisper.cpp', 'models'), path.join(home, '.local', 'share', 'whisper', 'models'));
...
function fontCandidates(platform = process.platform, env = process.env) {
  if (isWin(platform)) {
    ...
    return [
      path.join(win, 'Fonts', 'arialbd.ttf'),
      path.join(win, 'Fonts', 'arial.ttf'),
      path.join(win, 'Fonts', 'segoeuib.ttf'),
