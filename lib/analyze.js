...
export async function analyzeAudio(file, opts = {}) {
  ...
  const candidates = audio
    ? rankHighlights(env, [], duration, { count: 24, clip: 24, minGap: 12, idPrefix: 'c' })
    : [];
  ...
  return {
    ...
    highlights,
    candidates,
    // Video-derived fields stay empty until phase 2 lands; the canvas/panels tolerate these.
    sceneCuts: [],
    freezes: [],
    phantasm: [],
    phantasmStats: {
      ghostCount: 0, ghostDuration: 0, cutDuration: +duration.toFixed(1), riskyCount: 0,
