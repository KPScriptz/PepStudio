...
Style: Kin,Arial Black,${Math.round(fontSize * 1.3)},&H00FFFFFF,&H00000000,&H64000000,1,1,7,4,5,60,60,0
...
export async function generateCaptions(file, assPath, { range, clipRelative = false, punchy = false, kinetic = false } = {}) {
  const bin = whisperBin();
  const model = whisperModel();
  ...
  const key = capKey(file, ['gc', range ? range[0] : 0, range ? range[1] : '', clipRelative ? 1 : 0, punchy ? 1 : 0, kinetic ? 1 : 0]);
