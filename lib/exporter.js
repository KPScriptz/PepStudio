  ...
  try {
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      if (wmSpec) {
        ...
        parts.push(`[${ws}:v]scale=${wmSpec.w}:-2,format=rgba,colorchannelmixer=aa=${wmSpec.op}[wm]`);
      ...
      // Fades clamp to HALF the OUTPUT duration — a 3s fade on a clip sped up to 0.5s of output
      // otherwise ramps across the whole clip (in+out overlapping ≈ silence).
      const fIn = Math.max(0.004, Math.min(3, Number(seg.fadeIn) || 0, outDur / 2));
      const fOut = Math.max(0.004, Math.min(3, Number(seg.fadeOut) || 0, outDur / 2));
      ...
      // (built server-side from the word-level transcript). The ±40ms word-edge pad is applied in
      // OUTPUT time AFTER the speed conversion — padding in source time shrank it to 10ms on 4x
      // clips and ballooned it to 160ms (eating clean neighbors) on 0.25x slow-mo.
...
export async function mixMusicOnto(videoFile, musicPath, { volume = 0.15, duck = 'gentle', fadeOut = 3 } = {}) {
  ...
  const musicPrep = `[1:a]volume=${v},afade=t=out:st=${Math.max(0, dur - fade).toFixed(2)}:d=${fade},${AF}[m0]`;
  const graph = sc
