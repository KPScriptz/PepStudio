...
// A short, human chapter title from a clip's title or transcript snippet.
function clipTitle(clip, i) {
  const raw = String(clip.title || clip.snippet || '').replace(/\s+/g, ' ').trim();
  if (!raw) return `Moment ${i + 1}`;
  const words = raw.split(' ').slice(0, 7).join(' ').replace(/[\s.,;:!?-]+$/, '');
