...
import { binCandidates, binName, isWin, filterPath } from './platform.js';
...
// Prefer a libass-enabled build (keg-only ffmpeg-full on macOS) so we can burn captions, then an
// explicit override, then the platform's usual install locations, then whatever is on PATH.
// The bare fallback is the plain tool name: on Windows, spawn resolves it via PATHEXT.
function resolveBin(envVar, macFull, base) {
  if (process.env[envVar]) return process.env[envVar];
  if (!isWin() && fs.existsSync(macFull)) return macFull;
...
export const filterEscapePath = (p) => filterPath(p);
