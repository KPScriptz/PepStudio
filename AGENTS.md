...
Verified end-to-end on REAL footage (2026-07-08): a real 58s clip ran analyze->whisper->rank->export (2 real 1080x1920 mp4s); a real karaoke clip exposed + fixed a genuine gap — reactions.js now scores whisper non-speech annotations ((laughing)/(screaming)/(applause)), which were previously unscored (that clip went react 0->7.8). Also verified on synthetic footage: 4-pane NLE, project
...
PepAI titles + action-first AI Assistant (one-click actions wired to real pipelines: Remove Dead Air->banish, Rank->funny, Captions, Create Shorts->tiktok; real status+suggestions; NL chat demoted to Advanced Prompt), daily self-tuning trainer (creator-agnostic), editorial feedback recorder (real edit decisions -> data/feedback.jsonl via /api/feedback: clip_kept/dropped, segment_cut/restored, reorder, split, banish_all, ai_action_accepted, suggestion_followed — the corpus for future supervised learning) + story graph v0.1 (heuristic setup->payoff + callback detection from the whisper snippet; per-clip "why kept" line on highlight rows; decision records carry story context + dimensional scores). Story v0.2: per-clip confidence + competing interpretations + story-debt tracking + 5 scores (story/humor/context/callback/retention); a visual Clip Insight inspector (#clipInsight) renders the chain when a sequence clip is selected. One-Button Story Cut (#storyCutBtn): auto-assembles the sequence — keeps story clips (setup/payoff/callback) + strong reactions, drops filler, holds payoffs 1.2s, orders chronologically; sets keep/order on state.highlights and lets renderTracks rebuild seqMap (never overwrites it). "Edit for Me" (#autoEditBtn): assembles the best moments (shared assembleStoryCut brain) -> renders a tight highlight reel via /api/export/sequence with zoom:false (fast path: no whisper pass, no per-frame emphasis rescale) — NOT a de-silenced longcut (on a 68-min VOD that produced a useless 45-min render). Long cut (/api/export/longcut) without caption burn takes a zero-re-encode fast path: keyframe scan (2s) -> snap starts back to keyframes + merge -> per-segment `-c copy` extracts -> concat-copy (measured 24s for a 45-min keep from a 68-min 2.2GB VOD vs 10+ min re-encode; seams verified pixel-clean, cuts snap ~6s loose toward extra lead-in context; PEP_EXACT_CUT=1 or captions-on forces the frame-exact re-encode). Step 5 correction capture (#correctionShelf): after a MANUAL override (trim/keep/reorder/segment) a one-tap "why?" popover surfaces (Bad timing / Missed setup / Boring filler / Rhythm-pacing); tapping appends a human_correction token {action,clipId,story,reason} to feedback.jsonl. Auto-fades in 9s (optional/low-friction); base action already logged, Curate/Publish tabs,
FCP-magnetic timeline (V1 filmstrip thumbnails via /api/thumb, A1 audio waveforms from state.proj.envelope .v, click-to-select clips, HH:MM:SS:FF timecode). Full pipeline PROVEN on a real 68-min NBA 2K VOD (2026-07-09+): analyze (silenceMin 2.5/minSeg 1.5 -> 391 segs, was 661), Phantasm timeline render (divider gated to segs >=6px), Edit for Me (8-moment 4:00 reel), zero-re-encode long cut. Perf facts worth keeping: source keyframes ~6s apart, deep -ss seeks are FAST (0.5-2s, moov is faststart); a keyframe-only ffprobe packet scan of the 2.2GB file is 2s; sequence encode is ~3.3x realtime per clip standalone but ~3.4x SLOWER when N fast-seek inputs share one filter_complex (I/O contention) — per-clip temps + concat-copy (concat measured 1s) is the known-but-unbuilt fix. Both repos clean and pushed.
