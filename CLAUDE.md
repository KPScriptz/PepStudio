...
- **Track 2 (4-pane workspace):** ◐ **PARTIAL — the real NEXT task.** The picker (`#view-project-picker`) exists, but the full `#view-workspace` 4-pane NLE grid (Effect Controls / Program Monitor / Highlights / Track Lanes) is **not yet built** in `public/index.html`. This is the "deferred risky pane-relocation."
...
- **Track 5 (PepAI):** ◐ PARTIAL — `lib/pepai.js` uses a **local Ollama LLM**; `/api/status` shows `pepai.ready:false` until you install Ollama and pull a model (`OLLAMA_HOST` / `PEPAI_MODEL`). The daily `lib/trainer.js` → `data/gaming_heuristics.json` worker is **not present**; retention heuristics currently live in `lib/retention.js`.
