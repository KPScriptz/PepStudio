...
import { binCandidates, binName, whichCmd, whisperModelDirs, installHint, concatLine } from './platform.js';
...
// whisper.cpp ships under several names depending on how it was built/installed. Probe the
// platform's install dirs first, then fall back to an argv-only PATH lookup (`where` on Windows,
// `which` elsewhere — `command -v` is a POSIX shell builtin and needs a shell to run at all).
...
export function whisperBin() {
  ...
  for (const base of WHISPER_NAMES) {
    for (const c of binCandidates(base)) { if (fs.existsSync(c)) return c; }
  }
  for (const base of WHISPER_NAMES) {
    try {
      const w = whichCmd(binName(base));
      const out = execFileSync(w.cmd, w.args, { stdio: ['ignore', 'pipe', 'ignore'] }).toString().trim();
      const first = out.split(/\r?\n/)[0].trim();   // `where` can return several lines
...
// A SMALLER, faster model for the RANKING pass (keyword scoring only — accuracy barely matters, so
// tiny.en's ~3x speed is a free win). Falls back to the normal model if tiny.en isn't downloaded.
// Caption BURN-IN still uses whisperModel() for quality. WHISPER_FAST_MODEL overrides.
export function whisperFastModel() {
  if (process.env.WHISPER_FAST_MODEL && fs.existsSync(process.env.WHISPER_FAST_MODEL)) return process.env.WHISPER_FAST_MODEL;
  const base = whisperModel();
  const dirs = [base ? path.dirname(base) : null, path.join(os.homedir(), 'whisper.cpp', 'models'), path.join(PROJECT_ROOT, 'models')].filter(Boolean);
  for (const d of dirs) {
    const p = path.join(d, 'ggml-tiny.en.bin');
    if (fs.existsSync(p)) return p;
  }
  return base;
}
...
// tinydiarize model (speaker-turn detection). Like every whisper model here it is USER-installed —
// we resolve, never bundle. Skips the tiny for-tests stubs by requiring a real file size.
export const TDRZ_INSTALL = 'curl -L -o ~/whisper.cpp/models/ggml-small.en-tdrz.bin https://huggingface.co/akashmjn/tinydiarize-whisper.cpp/resolve/main/ggml-small.en-tdrz.bin';
export function whisperTdrzModel() {
  if (process.env.WHISPER_TDRZ_MODEL && fs.existsSync(process.env.WHISPER_TDRZ_MODEL)) return process.env.WHISPER_TDRZ_MODEL;
  const base = whisperModel();
  const dirs = [base ? path.dirname(base) : null, path.join(os.homedir(), 'whisper.cpp', 'models'), path.join(PROJECT_ROOT, 'models')].filter(Boolean);
  for (const d of dirs) {
    try {
      const hit = fs.readdirSync(d).find((f) => /tdrz.*\.bin$/i.test(f) && fs.statSync(path.join(d, f)).size > 50e6);
...
// Diarize one time range: tdrz transcription → segments split on [SPEAKER_TURN] markers into
// alternating speaker labels. Honest scope: tinydiarize detects TURNS (2-speaker alternation),
// not persistent voice identities — S1/S2 are turn-parity labels, stated in the UI.
export async function diarizeRange(file, [start, end]) {
  const bin = whisperBin();
  const model = whisperTdrzModel();
  if (!bin) throw new Error('whisper.cpp not found.');
  if (!model) { const e = new Error('TDRZ_MISSING'); e.code = 'TDRZ_MISSING'; throw e; }
  const tmp = await fsp.mkdtemp(path.join(os.tmpdir(), 'cf-diar-'));
  const wav = path.join(tmp, 'a.wav');
  const outBase = path.join(tmp, 'out');
  try {
    await extractWav(file, wav, [start, end]);
    await runWhisper(bin, model, wav, outBase, { tdrz: true });
    const json = JSON.parse(await fsp.readFile(`${outBase}.json`, 'utf8'));
    const out = [];
    let sp = 1;
    for (const seg of (json.transcription || [])) {
      // whisper.cpp's -oj emits the turn as a per-segment FIELD (`speaker_turn_next: true` =
      // the speaker changes AFTER this segment) — the [SPEAKER_TURN] text marker only appears
      // in console output. Parse the field; strip the marker defensively anyway.
      const raw = (seg.text || '').trim();
      const turn = seg.speaker_turn_next === true || /\[SPEAKER_TURN\]/i.test(raw);
      const text = raw.replace(/\[SPEAKER_TURN\]/gi, '').replace(/\s+/g, ' ').trim();
      if (text) out.push({
        t0: +(seg.offsets.from / 1000 + start).toFixed(2),
        t1: +(seg.offsets.to / 1000 + start).toFixed(2),
        speaker: `S${sp}`, text,
      });
      if (turn) sp = sp === 1 ? 2 : 1;
    }
...
function runWhisper(bin, model, wav, outBase, { threads, tdrz = false } = {}) {
  return new Promise((resolve, reject) => {
    ...
    // Per-call threads win over the env default so parallel chunks can each take a slice of the
    // cores without oversubscribing (e.g. 3 chunks × 2 threads on 6 perf cores).
    const t = String(threads || process.env.WHISPER_THREADS || '6');
    // tdrz (tinydiarize) runs in SEGMENT mode: -ml 1/-sow word-splitting suppresses the
    // [SPEAKER_TURN] markers, so diarization drops them and adds -tdrz instead.
    const args = tdrz
      ? ['-m', model, '-f', wav, '-oj', '-of', outBase, '-tdrz', '-bs', '1', '-bo', '1', '-t', t]
      : ['-m', model, '-f', wav, '-oj', '-of', outBase,
        '-ml', '1', '-sow', '-bs', '1', '-bo', '1', '-t', t];
...
// Kinetic (word-by-word) chunks: every spoken word is its own caption event, held until the
// next word starts (continuous hits — the TikTok/Hormozi style), with a short hold on pauses.
// Uses the SAME whisper word timestamps as everything else; no re-transcription.
function chunkKinetic(transcription, offset = 0) {
  const words = [];
  for (const seg of transcription) {
    const txt = (seg.text || '').trim();
    if (!txt || !/[a-z0-9]/i.test(txt)) continue;         // skip pure punctuation tokens
    if (/^[\[(].*[\])]$/.test(txt)) continue;             // skip whisper annotations: [INAUDIBLE], (laughing), [MUSIC]
    words.push({ t0: seg.offsets.from / 1000 + offset, t1: seg.offsets.to / 1000 + offset, w: txt });
  }
  return words.map((w, i) => {
    const next = words[i + 1];
    // hold to the next word when it follows within 0.6s; else linger 0.25s on the pause
    const end = next && next.t0 - w.t1 < 0.6 ? next.t0 : w.t1 + 0.25;
    return { start: +w.t0.toFixed(3), end: +Math.max(end, w.t0 + 0.12).toFixed(3), text: w.w, kinetic: true };
  });
}
...
// Caption style presets (Beta items 229-231). Each is a FULL 14-field Style line variant for the
// base (non-kinetic) chunks + a fill color the Kin style inherits, so kinetic words match the look:
//   pop     — the proven default: white Arial Black, outline+shadow, pop-in (unchanged).
//   bold    — "TikTok Bold": electric-yellow fill, thick 8px outline, hard shadow.
//   clean   — "YouTube Clean": white text on a semi-transparent dark box (BorderStyle 3), no outline.
//   minimal — "Minimal Studio": smaller white text, thin 2px outline, tucked to the bottom.
// Colors are ASS &HAABBGGRR. Emph keeps its amber accent in every preset.
export const CAPTION_PRESETS = {
  pop: { label: 'Punchy (default)', fill: '&H00FFFFFF', base: (fs) => `Style: Pop,Arial Black,${fs},&H00FFFFFF,&H00000000,&H64000000,1,1,6,3,2,80,80,260` },
  bold: { label: 'TikTok Bold', fill: '&H0000D7FF', base: (fs) => `Style: Pop,Arial Black,${fs},&H0000D7FF,&H00000000,&H96000000,1,1,8,4,2,70,70,250` },
  clean: { label: 'YouTube Clean', fill: '&H00FFFFFF', base: (fs) => `Style: Pop,Arial,${Math.round(fs * 0.82)},&H00FFFFFF,&H00000000,&H7A101010,0,3,0,0,2,90,90,240` },
  minimal: { label: 'Minimal Studio', fill: '&H00FFFFFF', base: (fs) => `Style: Pop,Arial,${Math.round(fs * 0.68)},&H00FFFFFF,&H00000000,&H64000000,1,1,2,1,2,100,100,200` },
};

export function buildAss(chunks, { w = 1080, h = 1920, fontSize = 96, preset = 'pop' } = {}) {
  const p = CAPTION_PRESETS[preset] || CAPTION_PRESETS.pop;
...
Style: Kin,Arial Black,${Math.round(fontSize * 1.3)},${p.fill},&H00000000,&H64000000,1,1,7,4,5,60,60,0

[Events]
Format: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text`;
  const lines = chunks.map((c) => {
    const txt = c.text.toUpperCase().replace(/\n/g, ' ');
    // Kinetic word: mid-screen (Alignment 5), hard fast pop — one word per event, no fade-out
    // (the next word lands on the same frame, so a fade would smear the hit).
    const fx = c.kinetic
      ? `{\\t(0,80,\\fscx126\\fscy126)\\t(80,150,\\fscx100\\fscy100)}`
    ...
    const style = c.kinetic ? 'Kin' : (c.emphasis ? 'Emph' : 'Pop');
    return `Dialogue: 0,${ts(c.start)},${ts(c.end)},${style},,0,0,0,,${fx}${txt}`;
  });
  return `${header}\n${lines.join('\n')}\n`;
}
...
export async function transcribeWindows(file, windows, { model } = {}) {
  ...
  // Parallelize across cores: split the windows into K groups, each with its OWN concat wav +
  // whisper process (group-relative offsets), run them concurrently, and remap each group locally.
  // ~Kx fewer wall-seconds than the old single serial pass. Threads are divided so K chunks don't
  // oversubscribe (e.g. 3 chunks × 2 threads on 6 perf cores). PEP_WHISPER_CHUNKS/CORES tune it.
  const CORES = Number(process.env.PEP_WHISPER_CORES) || 6;
  const K = Math.max(1, Math.min(Number(process.env.PEP_WHISPER_CHUNKS) || 3, segs.length));
  const perThreads = String(Math.max(1, Math.floor(CORES / K)));
  const groups = Array.from({ length: K }, () => []);
  segs.forEach((seg, i) => groups[i % K].push(seg));

  const transcribeGroup = async (group) => {
    if (!group.length) return;
    let acc = 0;
    for (const seg of group) { seg.cat = acc; acc += seg.dur; }   // group-relative concat offsets
    const tmp = await fsp.mkdtemp(path.join(os.tmpdir(), 'cf-batch-'));
    const wav = path.join(tmp, 'a.wav');
    const outBase = path.join(tmp, 'out');
    try {
      // Extract each window with a FAST input seek (-ss/-t BEFORE -i), then concat-copy into the
      // group wav. An atrim filtergraph has no input seek, so it decodes audio from 0 to the last
      // window (~63 min in on this VOD) just to grab ~60s: measured 8.0s vs 0.6s for seek+copy,
      // byte-identical output. `-t dur` is sample-exact at 16kHz, so the concat offsets (seg.cat,
      // built from seg.dur) stay aligned with the concatenated wav — the word remap is unchanged.
      const partFiles = [];
      for (let i = 0; i < group.length; i++) {
        const seg = group[i];
        const part = path.join(tmp, `w${i}.wav`);
        await ffmpeg(['-nostdin', '-y', '-ss', seg.s.toFixed(3), '-t', seg.dur.toFixed(3),
          '-i', file, '-ar', '16000', '-ac', '1', '-c:a', 'pcm_s16le', part]);
        partFiles.push(part);
      }
      if (partFiles.length === 1) {
        await fsp.copyFile(partFiles[0], wav);
      } else {
        const listFile = path.join(tmp, 'list.txt');
        await fsp.writeFile(listFile, partFiles.map((f) => concatLine(f)).join('\n'), 'utf8');
        await ffmpeg(['-nostdin', '-y', '-f', 'concat', '-safe', '0', '-i', listFile, '-c', 'copy', wav]);
      }
      await runWhisper(bin, mdl, wav, outBase, { threads: perThreads });
      const json = JSON.parse(await fsp.readFile(`${outBase}.json`, 'utf8'));
      for (const tok of (json.transcription || [])) {
        const txt = (tok.text || '').trim();
        if (!txt) continue;
        const ct = tok.offsets.from / 1000;
        const ct1 = tok.offsets.to / 1000;
        let seg = group[0];
        for (const cand of group) { if (ct + 1e-6 >= cand.cat) seg = cand; else break; }
  ...
  try {
    await Promise.all(groups.map(transcribeGroup));
...
export async function generateCaptions(file, assPath, { range, clipRelative = false, punchy = false, kinetic = false, stylePreset = 'pop' } = {}) {
  const bin = whisperBin();
  const model = whisperModel();
  if (!bin) throw new Error(`whisper.cpp not found. Install it: ${installHint('whisper-cli')}`);
  if (!model) throw new Error('No whisper model. Download one, e.g.:\n  curl -L -o models/ggml-base.en.bin https://huggingface.co/ggerganov/whisper.cpp/resolve/main/ggml-base.en.bin');

  const key = capKey(file, ['gc', range ? range[0] : 0, range ? range[1] : '', clipRelative ? 1 : 0, punchy ? 1 : 0, kinetic ? 1 : 0, stylePreset]);
  ...
  const chunks = kinetic ? chunkKinetic(json.transcription || [], offset)
    : punchy ? chunkPunchy(json.transcription || [], offset)
      : chunkTokens(json.transcription || [], 3, offset);
  await fsp.mkdir(path.dirname(assPath), { recursive: true });
  await fsp.writeFile(assPath, buildAss(chunks, { preset: stylePreset }), 'utf8');
