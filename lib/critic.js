...
export function auditCut(cut = {}, opts = {}) {
  ...
  // Score by AFFECTED TIME, not issue count. Counting issues saturated instantly: a cut with a
  // healthy 2.68 words/sec average but sixteen short quiet pockets scored 0 — identical to a cut
  // that is 74 seconds of silence. That makes the number useless for comparing two runs, which is
  // the main thing it's for. Each category is now proportional to the fraction of runtime it
  // spoils, and separately capped so one dimension can't consume the whole score.
  const affected = (type) => {
    const spans = issues.filter((i) => i.type === type)
      .map((i) => [Number(i.atSec) || 0, Math.max(Number(i.endSec ?? i.atSec) || 0, Number(i.atSec) || 0)])
      .sort((a, b) => a[0] - b[0]);
    let sec = 0, last = -Infinity;
    for (const [a, b] of spans) { const s = Math.max(a, last); if (b > s) { sec += b - s; last = b; } }
    return sec;
  };
  const frac = (sec) => (outSec > 0 ? sec / outSec : 0);
  const penalty =
    Math.min(35, frac(affected('dead_air')) * 300)              // dead air is the harshest signal
    + Math.min(30, frac(affected('low_speech_density')) * 60)
    + Math.min(20, frac(affected('no_pattern_interrupt')) * 40)
    + (issues.some((i) => i.type === 'weak_cold_open') ? 15 : 0);
