    ...
    const settled = await mapLimit(clips, PACK_CONCURRENCY, async (clip, i) => {
      try {
        ...
        if (wantCaps) {
          ...
          const { chunks } = await generateCaptions(file, ass, { range: [start, end], clipRelative: true, punchy: req.body.punchy !== false, kinetic: req.body.kinetic === true, stylePreset: req.body.capStyle || 'pop' });
          if (burn) subs = ass;
