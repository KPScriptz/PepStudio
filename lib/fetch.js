...
import { binCandidates, binName, whichCmd, installHint } from './platform.js';

// Locate yt-dlp: explicit override, then the platform's install locations, then a PATH lookup.
// The PATH lookup is argv-only (`where` on Windows, `which` elsewhere) — never a shell string.
export function ytdlpBin() {
  if (process.env.YTDLP_BIN && fs.existsSync(process.env.YTDLP_BIN)) return process.env.YTDLP_BIN;
  for (const c of binCandidates('yt-dlp')) { if (fs.existsSync(c)) return c; }
  try {
    const w = whichCmd(binName('yt-dlp'));
...
export function downloadUrl(url, destDir, { base = 'vod', onProgress, onStage } = {}) {
  return new Promise((resolve, reject) => {
    const bin = ytdlpBin();
    if (!bin) return reject(new Error(`yt-dlp not found. Install it: ${installHint('yt-dlp')}`));
