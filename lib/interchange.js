...
export function buildFcp7Xml(absPath, meta, segments, markers = [], { title = 'PepStudio Cut' } = {}) {
  ...
  // path.basename + pathToFileURL: split('/') left a whole Windows path as the 'name', and
  // 'file://localhost' + a backslash path produced a malformed URL Premiere/FCP can't relink.
  const name = path.basename(absPath);
  const url = pathToFileURL(absPath).href.replace('file://', 'file://localhost');
