...
// macOS ONLY: the helper is Swift against Apple's Vision/AppKit frameworks, compiled with swiftc.
// The .swift source ships inside the bundle, so an existsSync(SRC) check alone reported true on
// Windows and callers then threw at spawn — gate on the platform so game-event detection degrades
// cleanly (audio-only ranking) instead of failing the analyze.
export function ocrAvailable() {
  if (process.platform !== 'darwin') return false;
  return fs.existsSync(SRC) || fs.existsSync(BIN);
}
