...
export async function exportSequence(file, segments, outFile, { vertical = true, fps, draft = false, res = 0, crf = 0, gainDb = 0, lut = null, look = null, watermark = null, vstyle = 'crop' } = {}) {
  ...
  try {
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      if (vertical && vstyle === 'blur') {
        ...
        parts.push(`[0:v]setpts=PTS-STARTPTS,split=2[bf0${i}][bf1${i}]`);
        ...
        parts.push(`[0:v]setpts=PTS-STARTPTS,${base}${eqStep}${lutStep}format=yuv420p[vb]`);
      }
      let cur = 'vb';
      ...
      parts.push(`[0:a]asetpts=PTS-STARTPTS,afade=t=in:st=0:d=${fIn},afade=t=out:st=${Math.max(0, dur - fOut).toFixed(3)}:d=${fOut}${gain ? `,volume=${gain}dB` : ''}[as]`);
      const mixLabels = ['as'];
