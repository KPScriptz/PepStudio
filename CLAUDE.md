...
- **Parallel pack rendering:** `/api/export/tiktok`, `/api/export/shorts`, and `/api/export/thumbs` render clips CONCURRENTLY via `mapLimit(items, PACK_CONCURRENCY, …)` in `server.js` (per-clip whisper→zoom→encode pipelines are independent; `mkdtemp` per call verified collision-free). `PACK_CONCURRENCY` defaults to 3 (`PEP_PACK_CONCURRENCY` to override) — bounded so large packs don't oversubscribe the 6 perf cores. Re-measured 2026-07-10: 3 captioned+zoomed verticals in **4.9s** wall (was 14s) after the greedy-whisper + hardware-decode changes below.
- **Hardware DECODE (benchmarked, not assumed):** every video-decoding render (`exportShort/exportLongCut/exportSequence/burnSubs/grabFrame`) prepends `-hwaccel videotoolbox` before its source `-i` (`HWDECODE` const in `lib/exporter.js`), offloading H.264/HEVC decode to the media engine so CPU stays free for the software filter chains + concurrent whisper. Frames auto-download to system memory for filters. Independent of the ENCODER policy above (x264 still default). `PEP_HWDECODE=off` reverts to software decode.
- **Whisper decode:** greedy (`-bs 1 -bo 1`) not the whisper.cpp default beam-5/best-of-5 — ~2-3x faster with no quality loss on the short punchy 1-2 word captions. Flash-attn is on by default in this build. `-ml 1 -sow` retained for token-level word timing (zoom/emphasis engine needs it). `WHISPER_THREADS` overrides (default 6 = perf cores). See `lib/captions.js` `runWhisper()`.
- **Ranking transcription is parallel + fast-model (2026-07-14):** `transcribeWindows` splits the gated windows into `PEP_WHISPER_CHUNKS` (default 3) groups, each its own concat wav + whisper process running concurrently with `floor(cores/K)` threads each (group-relative offset remap — verified: words land in the correct window). The funny/storyboard route feeds it `whisperFastModel()` (tiny.en if present, else base) since ranking only needs keyword accuracy. **Measured 2.24x** end-to-end on 4-min audio (base-serial → tiny-parallel); more on the 25-min storyboard pass. Caption BURN-IN still uses base.en via `whisperModel()`.
- **YouTube cut = ONE video encode:** `/api/export/youtube` transcribes the cut from an audio-only concat extraction (`generateCutCaptions` in `lib/captions.js` — no video frames decoded), whose 0-based timing matches `concatGraph`, then concats + burns captions in a single `exportLongCut` pass. The old path rendered a throwaway `youtube_raw.mp4`, transcribed it, then re-encoded to burn (two full video passes) — halved. Verified: hook + 2 segments → exact 18.0s output, captions aligned, no `youtube_raw`.
- **Robustness & safety hardening (2026-07-10, audited + live-verified):**
  - `/api/reveal` uses `execFile('open', ['-R', p])` (no shell) — a filename with backticks/`$()` can't inject. Never rebuild this as `exec(\`...\`)`.
  - `/api/video` range parser handles all RFC 7233 forms (`N-M`, `N-`, suffix `-N`), clamps to file size, answers out-of-range with **416**, and pipes through `pipeRange()` which has a stream `error` handler. Previously threw on suffix ranges (null-match `m[1]`) and could crash the process on a read error.
  - **Packs are partial-success:** tiktok/shorts/thumbs isolate each item in try/catch and return survivors in `clips`/`shorts`/`thumbs` plus a `failed:[{clip,error}]` list; only an all-fail batch 500s. One bad clip no longer discards a whole pack's renders (this also neutralized the old orphaned-sibling-process issue).
  - **Input validation:** `cleanRange()` (server) drops NaN/negative/reversed clip ranges before ffmpeg; `exportLongCut` clamps `start≥0`; drawtext overlay `fontColor/x/y/fontSize` are sanitized in `lib/exporter.js` (`safeColor/safePos/safeSize`) so client style values can't corrupt the `filter_complex` graph.
  - **Resource bounds:** `MAX_BATCH=200` (`PEP_MAX_BATCH`) caps clips/segments/times per request; `grabFrame` renders to a temp file then atomic-renames (no half-written thumb served); on-demand `/api/thumb` grabs run through `thumbGate` (concurrency-capped); the `jobs` map is pruned to 50 (drops oldest terminal jobs).
- **Caption cache (content-addressed):** `generateCaptions`/`generateCutCaptions` key transcription on `sha1(resolvedSourcePath + mtime + model + range/flags)` and cache `.ass/.srt/chunks` under `PEP_CAPCACHE` (default `$TMPDIR/pep-capcache`). Re-exporting the same clip skips whisper+wav-extract entirely — measured captions-only route **0.85s → 0.01s** (85x), byte-identical output. Source mtime in the key auto-invalidates on edit. Cache writes are best-effort (never fail an export). Bounded to `PEP_CAPCACHE_MAX` (default 300) entries via `capCachePrune()` (oldest-by-mtime eviction, removes all 3 files per key). Lives in `lib/captions.js`.
- **Disk-space guard:** `/api/export/*` is fronted by middleware that `statfs`-checks the render volume and returns **507** if free space < `PEP_MIN_FREE_GB` (default 2GB) — refuses up front instead of filling the disk and dying mid-encode. statfs failure never blocks.
- **Subprocess niceness:** `lib/ff.js` `deprioritize(pid)` sets every spawned ffmpeg/whisper to nice **+5** (`PEP_NICE` override, `0` disables) so the Node event loop (status polls) and Electron UI stay responsive during a big export. Verified: spawned ffmpeg shows `NI 5` in `ps`. Positive nice needs no privileges.
- **Timeline frontend perf (2026-07-10, live-profiled):** `.lane` gets `contain: layout paint style` (each lane's clip reflow/repaint can't cascade to siblings — safe, `.tblk` stay in-box); the `window resize` handler coalesces a burst into ONE rAF redraw (`_resizeRaf` guard in `public/app.js`) instead of a full heavy `draw()` per event. Verified in a real viewport: 5 lanes / 16 blocks render clean, `getComputedStyle(.lane).contain === 'content'`, 8 resize events → 1 frame. NOT done (measured no-ops for this code, don't cargo-cult them): playhead `transform` rewrite (updates at ~4Hz via `timeupdate`, not 60fps), passive listeners (no wheel/touch listeners exist), `will-change`/`translate3d` on every `.tblk` (anti-pattern — they deliberately limit compositing layers, see the backdrop-filter note in style.css).
...
- **Tuned local model (2026-07-02):** `pepstudio-brain` (see `./Modelfile`: FROM llama3.2:3b, num_ctx 8192, temp .65/top_p .9/repeat 1.15, num_predict 256, no-emoji baseline SYSTEM; rebuild with `ollama create pepstudio-brain -f Modelfile`). `lib/pepai.js` prefers it automatically (prefix-aware; PEPAI_MODEL overrides) and sends `keep_alive:"24h"` on every call so the model stays GPU-resident (measured: warm chat 1s). Weights blob is shared with llama3.2:3b — no extra disk. Trainer: normalized trigger dedupe, z clamped 1.5-4, `schemaVersion:2`.
- **FCP-magnetic sequence lanes (2026-07-02):** the 5 sequence lanes are restyled (dark, CSS-only) into a Final Cut reading — V1 clip lane is a thick 40px **primary spine** (cobalt), V2 overlay lane slim/connected above, A1/A2/A3 slim green audio roles below. Lane heights are INTENTIONALLY unequal (22/40/22/22/22) and `.trackHeaders .trk` rows are resized to match — not a bug. Purely `[data-theme="dark"]` CSS: `renderTracks`/`state.seqMap`/playhead/drag-scrub/Phantasm untouched; CHROME DEEP heat top-edges preserved. Rejected the pasted `refreshTimelineTracks` rewrite (gated on `audioScore>80` which never matches real ~0-3 scores, used nonexistent `--tier-*`/`state.activeClipId`/`overlay.start`, and its footer replacement deleted #timeline/#banishBtn/#phantasmSummary).
