...
export function consumeFeedback({
  ...
  let cursor = h.feedbackConsumedLines || 0;
  // TRUNCATION GUARD: the cursor is a line COUNT, so if the log is ever rotated, truncated, or
  // moved to a fresh data dir, it can point past the end of a now-shorter file — and slice()
  // silently returns nothing. The loop would then ignore every future correction forever while
  // cheerfully reporting "no new corrections". Treat a cursor beyond EOF as a new log and start
  // over rather than going quietly deaf.
  let rewound = false;
  if (cursor > lines.length) { cursor = 0; rewound = true; }
  ...
  if (total === 0) {
    ...
    return { ok: true, applied: false, counts, rewound, reason: 'no new corrections' };
  }
  ...
  return {
    ok: true, applied: true, counts, rewound,
    changes: {
