...
export async function exportSequence(file, segments, outFile, { vertical = true, fps, draft = false, res = 0, crf = 0, gainDb = 0, lut = null } = {}) {
  ...
  try {
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      parts.push(`[0:v]setpts=PTS-STARTPTS,${base}${lutStep}format=yuv420p[vb]`);
      let cur = 'vb';
