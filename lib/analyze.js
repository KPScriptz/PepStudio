...
async function audioPass(file, { silenceDb = -35, silenceMin = 1.5 } = {}) {
  let stderr = '';
...
function buildPhantasm(silences, freezes, sceneCuts, duration, { minSeg = 0.8 } = {}) {
  const step = 0.2;
...
function activeSegments(silences, duration, pad = 0.25, minLen = 0.8) {
  const segs = [];
  let cur = 0;
...
// Rank highlight moments by combining loudness peaks with scene-cut density.
function rankHighlights(env, cuts, duration, { count = 8, clip = 30, minGap = 18 } = {}) {
  ...
  // Score each bucket; smooth loudness over ~2s.
  ...
  const scored = env.map((e, i) => {
    ...
    const loudZ = ((s / k) - mean) / sd;
    return { t: e.t, score: loudZ + 0.6 * sceneDensity(e.t) };
  });
...
export async function analyze(file, opts = {}) {
  ...
  const highlights = audio ? rankHighlights(env, cuts, duration, opts) : [];
  const phantasm = buildPhantasm(silences, freezes, cuts, duration, opts);

  // Phantasm roll-up: how much is ghosted, and the resulting cut length.
  ...
  return {
    ...
    phantasmStats: {
      ...
      thresholds: {
        audioDb: opts.silenceDb ?? -35, audioMin: opts.silenceMin ?? 1.5,
